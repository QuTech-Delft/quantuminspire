...
class QIBackend(BasicEngine):
        ...
        for state in filtered_states:
            ...
            for i in range(len(qureg)):
                ...
                if int(state) & (1<<physical_id):
                    ...
                    mapped_state += '0'
            probability = filtered_states[state]
    ...
    def _run(self):
        ...
        #    raise ProjectQBackendError("Failed to run the circuit. Aborting.") from ex
    ...
    def _filter_histogram(histogram, mask_bits):
        ...
        Keys in the dict are the states represented as an integer number (may be int or string), values the
        probability corresponding to that state.

        The list specifies the relevant bits, any bit not set to 1 in the mask will be ignored, and the resulting
        state probabilities of equivalent states summed.
    ...
    def _int_state_as_binary_string(self, state):
        return ("{0:0%db}" % self._number_of_qubits).format(state)
