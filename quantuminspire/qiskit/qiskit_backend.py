...
# note: Rz(theta)=\exp(-i theta/2 Z)
    ...
    def run_circuit(self, circuit):
        ...
        nshots=self._shots
        print('run_circuit: nshots %d' % (nshots,))
        self._cqasm = self.generate_cqasm(ccircuit)
        
        if 1:
            # execute cqasm
            results=self.qi_api.execute_qasm(self._cqasm, nshots=nshots)
            self._qi_results= results
            self._qi_last_job = self.qi_api._last_job
            
            counts=copy.copy(results['histogram'])
            for k in counts:
                counts[k]=counts[k]*nshots
