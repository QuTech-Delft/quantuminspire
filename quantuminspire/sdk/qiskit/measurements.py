...
class Measurements:  # pylint: disable=too-many-instance-attributes
    """The Measurements class registers measurements and the translations of measurements of qubits to classical
    registers."""

    def __init__(self) -> None:
        """_measurements_reg and _measurements_state are list of lists, for each measurement the lists contains a list
        of [qubit_index, classical_bit_index], which represents the measurement of a qubit to a classical bit.
    ...
    def from_experiment(cls, experiment: QasmQobjExperiment) -> Measurements:
        ...
        for instruction in experiment.instructions:
            if instruction.name == "measure":
                ...
                instance.measurements_state.append(
                    [
                        instance.nr_of_qubits - 1 - instruction.qubits[0],
        ...
        for q1, c1 in self._measurements_reg:
            if q1 == creg:
                raise QiskitBackendError(
                    f"Classical bit {creg} used in a conditional gate is not measured "
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        :return: Dict containing the measurement list with positional indexes in the resulting qubit_state and
            classical_state and the number of classical bits.
        """
        return {
            "measurements_state": self._measurements_state,
    ...
    def validate_number_of_clbits(self) -> None:
        ...
        if self.max_measurement_index >= self._number_of_clbits:
            raise QiskitBackendError(
                f"Number of classical bits ({self._number_of_clbits}) is not sufficient for "
    ...
    def validate_unsupported_measurements(self) -> None:
        ...
        for q1, c1 in self._measurements_reg:
            for q2, c2 in self._measurements_reg:
                if q1 == q2 and c1 != c2:
                    raise QiskitBackendError(
                        f"Measurement of qubit {q1} to different classical registers is not " f"supported"
                    )
                if q1 != q2 and c1 == c2:
                    raise QiskitBackendError(
                        f"Measurement of different qubits to the same classical register {c1} " f"is not supported"
                    )
    ...
    def qubit_to_classical_hex(self, qubit_register: str) -> str:
        ...
        :return: The hexadecimal value of the classical state.
        """
        qubit_state = f"{int(qubit_register):0{self.nr_of_qubits}b}"
